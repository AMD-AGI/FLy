...
import fly.api.metrics
import fly.api.registry
import fly.api.task
import fly.models
from fly.caching.cache import delete_cache
...
from fly.loggers import EvaluationTracker
from fly.loggers.utils import add_env_info, add_tokenizer_info, get_git_commit_hash
from fly.tasks import TaskManager, get_task_dict
from fly.utils import (
...
if TYPE_CHECKING:
    from fly.api.model import LM
    from fly.api.task import Task
...
def simple_evaluate(
    ...
    :param model: Union[str, LM]
        Name of model or LM object, see fly.models.get_model
    ...
    if isinstance(model, str):
        ...
        if isinstance(model_args, dict):
            ...
            lm = fly.api.registry.get_model(model).create_from_arg_obj(
                model_args,
                {
            ...
            lm = fly.api.registry.get_model(model).create_from_arg_string(
                model_args,
                {
                    "batch_size": batch_size,
                    "max_batch_size": max_batch_size,
        ...
        if not isinstance(model, fly.api.model.LM):
            raise TypeError(
                f"The value of `model` passed to simple_evaluate() was of type {type(model)}, but is required to be a subclass of fly.api.model.LM . This may be because you are passing an initialized Hugging Face PreTrainedModel without having wrapped it in `fly.models.huggingface.HFLM(pretrained=my_model)` first."
            )
    ...
    if lm.rank == 0:
        ...
        if isinstance(lm, fly.models.huggingface.HFLM):
