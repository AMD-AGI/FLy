...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="task1,task2",
        help="Comma-separated list of task names or task groupings to evaluate on.\nTo get full list of tasks, use one of the commands `fly --tasks {{list_groups,list_subtasks,list_tags,list}}` to list out all available names for task groupings; only (sub)tasks; tags; or all of the above",
    )
    parser.add_argument(
        "--model_args",
        "-a",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma separated string arguments passed to wandb.init, e.g. `project=fly,job_type=eval",
    )
    parser.add_argument(
        "--wandb_config_args",
        type=str,
...
def cli_evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    # defer loading `fly` submodules for faster CLI load
    from fly import evaluator, utils
    from fly.evaluator import request_caching_arg_to_dict
    from fly.loggers import EvaluationTracker, WandbLogger
    from fly.tasks import TaskManager
        ...
        if os.path.isdir(args.tasks):
            ...
            if task_missing:
                ...
                eval_logger.error(
                    ...
                    f"{utils.SPACING}Try `fly --tasks list` for list of available tasks",
                )
                raise ValueError(
                    f"Tasks not found: {missing}. Try `fly --tasks {{list_groups,list_subtasks,list_tags,list}}` to list out all available names for task groupings; only (sub)tasks; tags; or all of the above, or pass '--verbosity DEBUG' to troubleshoot task registration issues."
                )
    ...
    if args.trust_remote_code:
        ...
        # script--`datasets` gets imported by fly internally before these lines can update the env.
        import datasets
