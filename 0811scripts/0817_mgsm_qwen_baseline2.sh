...
do
    python -m lm_eval \
        --model hf \
        --model_args pretrained=${draft_model},parallelize=${para},target_model_path=${target_model},use_sd=True,spd_k=0,total_gen_tok=4096 \
        --tasks ${task} \
        ...
        &> ./output_log/${data}_4_Qwen25Baseline7b_${task}
done
