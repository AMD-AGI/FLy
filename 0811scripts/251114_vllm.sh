...
enable_fly=True
fly_win_len=8
entropy_threshold=None  # Set to a float value if needed, e.g., 0.5
...
python -m lm_eval \
    --model vllm \
    --model_args pretrained=${target_model},enable_fly=${enable_fly},fly_win_len=${fly_win_len},entropy_threshold=${entropy_threshold} \
    --tasks ${task} \
    ...
    &> ./output_log/${data}_${task}_vllm_FLy.log
